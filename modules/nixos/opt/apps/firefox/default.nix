...
with lib.nerv;
let
  cfg = config.nerv.opt.apps.firefox;
  ...
  # See https://github.com/sherubthakur/dotfiles
  strictSettings = merge [
  ...
  extensions = with pkgs.nur.repos.rycee.firefox-addons; [
    ublock-origin
    clearurls # Removes tracking elements from URLs (utm)
    darkreader
    sponsorblock # Skip Sponsorships on YouTube
    facebook-container # Prevent Facebook from tracking you around the web.
    sidebery # Ctrl+e - toggle left sidebar
    browserpass
...
in
{
  options.nerv.opt.apps.firefox = with types; {
    ...
    search = mkOpt attrs defaultSearch "Search settings to apply to the profile.";
    # extensions = mkOpt attrs defaultExtensions "Extensions to apply to the profile.";
  };

  config = mkIf cfg.enable {
    nerv.opt.desktop.addons.firefox-nordic-theme = on;

    services.gnome.gnome-browser-connector.enable = config.nerv.opt.desktop.gnome.enable;
    ...
    nerv.home.extraOptions = {
      programs.firefox = {
        ...
        profiles.${config.nerv.opt.user.name} = {
          inherit (cfg) extraConfig userChrome search;
          extensions.packages = extensions;
          settings = merge [ (import ./config-annoyances.nix) ];
          id = 0;
          name = config.nerv.opt.user.name;
        };
      ...
      home.packages = with pkgs; [
        (makeDesktopItem {
          ...
          genericName = "Open firefox by using Work profile";
          icon = "firefox";
          exec = ''${pkg-firefox}/bin/firefox -P work'';
          categories = [ "Network" ];
        })
        (makeDesktopItem {
          name = "firefox-other-profile";
          desktopName = "Firefox Other (strict privacy) profile";
          genericName = "Open firefox by using Other (not strict) profile";
          icon = "firefox";
          exec = ''${pkg-firefox}/bin/firefox -P other'';
          categories = [ "Network" ];
        })
      ];
    };

    nerv.opt.persist.state.homeDirectories = [ ".mozilla/firefox" ];
  };
}
