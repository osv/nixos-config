...
with lib.nerv;
let
  cfg = config.nerv.opt.apps.discord;
...
in
{
  options.nerv.opt.apps.discord = with types; {
  ...
  config = mkIf (cfg.enable or cfg.chromium.enable) {
    environment.systemPackages =
      ...
      ++ lib.optional cfg.canary.enable pkgs.nerv.discord
      ++ lib.optional cfg.chromium.enable pkgs.nerv.discord-chromium
      ++ lib.optional cfg.firefox.enable pkgs.nerv.discord-firefox
