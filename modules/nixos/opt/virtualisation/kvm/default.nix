...
with lib.nerv;
let
  cfg = config.nerv.opt.virtualisation.kvm;
  user = config.nerv.opt.user;
in
{
  options.nerv.opt.virtualisation.kvm = with types; {
  ...
  config = mkIf cfg.enable {
    ...
    nerv = {
      opt.user = { extraGroups = [ "qemu-libvirtd" "libvirtd" "disk" ]; };

      opt.apps = { looking-glass-client = on; };
      ...
      opt.persist.state.directories = [ "/var/lib/libvirt" ];
    };
  };
}
