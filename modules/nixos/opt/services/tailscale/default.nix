...
with lib;
with lib.nerv;
let cfg = config.nerv.opt.services.tailscale;
in
{
  options.nerv.opt.services.tailscale = with types; {
    ...
    assertions = [
      {
        ...
        message = "nerv.opt.services.tailscale.autoconnect.key must be set";
      }
    ];
