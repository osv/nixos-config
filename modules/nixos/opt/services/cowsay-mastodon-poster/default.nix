...
let
  ...
  inherit (lib.nerv) mkBoolOpt mkOpt;
  inherit (pkgs) fortune toot;
  inherit (pkgs.snowfallorg) cow2img;

  cfg = config.nerv.opt.services.cowsay-mastodon-poster;
...
in
{
  options.nerv.opt.services.cowsay-mastodon-poster = with types; {
    enable = mkBoolOpt false "Whether or not to enable cowsay posts.";
    short = mkBoolOpt false "Use short fortunes only.";
    user = mkOpt str config.nerv.opt.user.name "The user to run as.";
