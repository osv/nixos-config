...
with lib.nerv;
let
  cfg = config.nerv.opt.services.openssh;
  ...
  other-hosts = lib.filterAttrs
    (key: host:
      key != name && (host.config.nerv.opt.user.name or null) != null)
  ...
  other-hosts-config = lib.concatMapStringsSep
    ...
    (name:
      let
        ...
        remote-user-name = remote.config.nerv.opt.user.name;
...
in
{
  options.nerv.opt.services.openssh = with types; {
  ...
  config = mkIf cfg.enable {
    ...
    nerv = {
      opt.persist.state = {
        ...
        homeDirectories = [ { directory = ".ssh"; mode = "0700"; } ];
      };

      opt.user.extraOptions.openssh.authorizedKeys.keys =
        cfg.authorizedKeys;
