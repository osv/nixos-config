...
let
  ...
  cfg = config.nerv.opt.services.writefreely;
  ...
  withSqlite = text:
    ...
    '';
in {
  options.nerv.opt.services.writefreely = {
    enable =
    ...
    database = {
      ...
      createLocally = mkOption {
        ...
        default = false;
        description = ''
          When <option>nerv.opt.services.writefreely.database.type</option> is set to
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        assertion = cfg.host != "";
        message = "nerv.opt.services.writefreely.host must be set";
      }
      {
        assertion = isMysqlLocal -> cfg.database.passwordFile != null;
        message =
          "nerv.opt.services.writefreely.database.passwordFile must be set if nerv.opt.services.writefreely.database.createLocally is set to true";
      }
      {
        assertion = isSqlite -> !cfg.database.createLocally;
        message =
          "nerv.opt.services.writefreely.database.createLocally has no use when nerv.opt.services.writefreely.database.type is set to sqlite3";
      }
    ];
