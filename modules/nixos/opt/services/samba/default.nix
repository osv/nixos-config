...
let
  cfg = config.nerv.opt.services.samba;
  ...
  shares-submodule = with types; submodule ({ name, ... }: {
    options = {
      ...
      only-owner-editable = mkBoolOpt false "Whether the share is only writable by the system owner (nerv.opt.user.name).";
...
in
{
  options.nerv.opt.services.samba = with types; {
    ...
    services.samba = {
      ...
      shares = mapAttrs
        (name: value: {
          ...
          "write list" = config.nerv.opt.user.name;
